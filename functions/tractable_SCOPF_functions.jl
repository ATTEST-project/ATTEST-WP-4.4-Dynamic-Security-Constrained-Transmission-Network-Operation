...
return v_sq_c, teta_c, Pg_c, Qg_c, pen_ws_c, pen_lsh_c, p_fl_inc_c, p_fl_dec_c,p_ch_c,p_dis_c,soc_c
            ...
            for j in 1:size(node_data[i,1].node_num,1)                                                # length of each node vector in 'node_data' variable
              ...
              qloss           = @expression(model_name,      lf_tetaq+lf_voltq)
               ...
               +qloss
                                        ...
                                        push!(ploss_dic,  [t,idx_nd_nw_buses,idx_cnctd_nd] => lf_voltp) #becasue the vol angle term should be positive
        ...
        if ~isempty(node_data_trans[i,1].node_num)
            for j in 1:size(node_data_trans[i,1].node_num,1)                                                # length of each node vector in 'node_data' variable
              ...
              +tratio*qloss
               ...
               push!(ploss_trans_dic,  [t,idx_nd_nw_buses,idx_cnctd_nd_trans] => lf_voltp)
           elseif loss_indicator==0
               pijt1 =   @expression(model_name,pinj_ij_sh+pinj_ij_sr1
               # +tratio*ploss
               )
...
function branch_violation_percent(indicator_iter,br_viol_perc_dic,br_viol_perc_c_dic,coeff_nr,coeff_c,coeff_compl)
    if indicator_iter==3
    ...
    for  t in 1:nTP
     ...
     push!(br_viol_perc_dic_iter, [t,idx_to_line[k],idx_from_line[k]] => (branch_flow_check[t][3][1][j[1]]-1.0))
       elseif  haskey(br_viol_perc_dic,[t,idx_from_line[k],idx_to_line[k]])
           ...
           # push!(br_viol_perc_dic, [t,idx_from_line[k],idx_to_line[k]] => coeff_compl*(br_viol_perc_dic[[t,idx_from_line[k],idx_to_line[k]]]))
           # push!(br_viol_perc_dic, [t,idx_to_line[k],idx_from_line[k]] => coeff_compl*(br_viol_perc_dic[[t,idx_to_line[k],idx_from_line[k]]]))
           # push!(br_viol_perc_dic, [t,idx_from_line[k],idx_to_line[k]] => coeff_compl*(branch_flow_check[t][3][1][j[1]]-1.0))
           # push!(br_viol_perc_dic, [t,idx_to_line[k],idx_from_line[k]] => coeff_compl*(branch_flow_check[t][3][1][j[1]]-1.0))
    ...
    for c in 1:nCont, s in 1:nSc, t in 1:nTP
     ...
     push!(br_viol_perc_c_dic_iter, [c,s,t,idx_to_line_c[c][k],idx_from_line_c[c][k]] =>(branch_flow_check_c[c,s,t][3][j[1]]-1))
                elseif haskey(br_viol_perc_c_dic,[c,s,t,idx_from_line_c[c][k],idx_to_line_c[c][k]])
                    ...
                    # push!(br_viol_perc_c_dic, [c,s,t,idx_from_line_c[c][k],idx_to_line_c[c][k]] =>coeff_compl*br_viol_perc_c_dic[[c,s,t,idx_from_line_c[c][k],idx_to_line_c[c][k]]])
                    # push!(br_viol_perc_c_dic, [c,s,t,idx_to_line_c[c][k],idx_from_line_c[c][k]] =>coeff_compl*br_viol_perc_c_dic[[c,s,t,idx_to_line_c[c][k],idx_from_line_c[c][k]]])
                    # push!(br_viol_perc_c_dic, [c,s,t,idx_from_line_c[c][k],idx_to_line_c[c][k]] =>coeff_compl*(branch_flow_check_c[c,s,t][3][j[1]]-1))
                    # push!(br_viol_perc_c_dic, [c,s,t,idx_to_line_c[c][k],idx_from_line_c[c][k]] =>coeff_compl*(branch_flow_check_c[c,s,t][3][j[1]]-1))
...
function voltage_violation_value(indicator_iter,coeff_nr,coeff_c,coeff_compl,voltage_violation)
    if indicator_iter==3
...
elseif haskey(volt_viol_normal,[t,b]) && haskey(voltage_violation["vol_viol_val_dic_min"],[t,b])
    if volt_viol_normal[[t,b]]<nw_buses[b].bus_vmin
        # push!(voltage_violation["vol_viol_val_dic_min"], [t,b] => coeff_compl*(nw_buses[b].bus_vmin-volt_viol_normal[[t,b]]))
        # push!(voltage_violation["vol_viol_val_dic_min"], [t,b] => coeff_compl*(voltage_violation["vol_viol_val_dic_min"][[t,b]]) )
    ...
    elseif volt_viol_normal[[t,b]]>nw_buses[b].bus_vmax
        ...
        # push!(voltage_violation["vol_viol_val_dic_max"], [t,b] =>coeff_compl* (voltage_violation["vol_viol_val_dic_max"][[t,b]]))
        # push!(voltage_violation["vol_viol_val_dic_max"], [t,b] =>coeff_compl* (volt_viol_normal[[t,b]]-nw_buses[b].bus_vmax))
        push!(vol_viol_val_dic_min_iter, [t,b] => 0)
        push!(vol_viol_val_dic_max_iter, [t,b] => (volt_viol_normal[[t,b]]-nw_buses[b].bus_vmax))
...
elseif haskey(volt_viol_contin,[c,s,t,b]) && haskey(voltage_violation["vol_viol_val_c_dic_min"], [c,s,t,b] )
    if volt_viol_contin[[c,s,t,b]]<nw_buses[b].bus_vmin-v_relax_factor_min
        # push!(voltage_violation["vol_viol_val_c_dic_min"], [c,s,t,b] =>coeff_compl*(nw_buses[b].bus_vmin-v_relax_factor_min-volt_viol_contin[[c,s,t,b]]))
        # push!(voltage_violation["vol_viol_val_c_dic_min"], [c,s,t,b] =>coeff_compl*(voltage_violation["vol_viol_val_c_dic_min"][[c,s,t,b]] ))
        push!(voltage_violation["vol_viol_val_c_dic_min"], [c,s,t,b] =>coeff_compl*(voltage_violation["vol_viol_val_c_dic_min"][[c,s,t,b]] + (nw_buses[b].bus_vmin-v_relax_factor_min-volt_viol_contin[[c,s,t,b]])))
    ...
    elseif volt_viol_contin[[c,s,t,b]]>nw_buses[b].bus_vmax+v_relax_factor_max
        push!(voltage_violation["vol_viol_val_c_dic_min"], [c,s,t,b] =>0)
        push!(voltage_violation["vol_viol_val_c_dic_max"], [c,s,t,b] =>coeff_compl*(voltage_violation["vol_viol_val_c_dic_max"][[c,s,t,b]] + (volt_viol_contin[[c,s,t,b]]-nw_buses[b].bus_vmax-v_relax_factor_max)))
        # push!(voltage_violation["vol_viol_val_c_dic_max"], [c,s,t,b] =>coeff_compl*(voltage_violation["vol_viol_val_c_dic_max"][[c,s,t,b]] ))
        # push!(voltage_violation["vol_viol_val_c_dic_max"], [c,s,t,b] =>coeff_compl*(volt_viol_contin[[c,s,t,b]]-nw_buses[b].bus_vmax-v_relax_factor_max))
...
(pinj_dict_c, pinj_dict_l_c, ploss_c_dict, qinj_dict_c,qinj_dict_l_c,ploss_trans_dic_c)    = line_expression_t_c(model_name,from_to_map_t,state_dependent)
...
# set_optimizer_attribute(model_name, "CPX_PARAM_BARSTARTALG", 3)#2,3,4
# set_optimizer_attribute(model_name, "CPX_PARAM_DEPIND", 1)
# set_optimizer_attribute(model_name, "CPX_PARAM_BARCROSSALG", 2)
# CPX_PARAM_DEPIND to 1
# CPX_PARAM_BARCROSSALG 1primal and 2 duall crossover




# set_optimizer_attribute(model_name, "CPX_PARAM_DEPIND",1)
# set_optimizer_attribute(model_name, "CPX_PARAM_BARCROSSALG",2)#(no improve)Decides which crossover is performed at the end of a barrier optimization.  0 1 2
# set_optimizer_attribute(model_name, "CPX_PARAM_BARGROWTH", 1e2)
...
set_optimizer_attribute(model_name, "CPX_PARAM_LPMETHOD",4)
set_optimizer_attribute(model_name, "CPX_PARAM_SOLUTIONTYPE", 2)
set_optimizer_attribute(model_name, "CPX_PARAM_BAREPCOMP", 1e-4)
set_optimizer_attribute(model_name, "CPX_PARAM_BARORDER", 1)# 1(25 to 16),2(no improve),3
set_optimizer_attribute(model_name, "CPX_PARAM_BARSTARTALG", 4)#2,3,4
