...
elseif  type_post_PF=="SA"
    ...
    nd_shunt_c          =shunt_c[1]
    # if !isnothing(variable_by_name.(model_name, "lsh[$i,$j,$k]" for i in 1:nSc, j in 1:nTP, k in 1:nLoads)[1])
    if !isnothing(variable_by_name(model_name, "pen_ws[1,1,1]"))
        nd_ren_curt_c       =JuMP.value.(pen_ws[:,:,:])
    else
        nd_ren_curt_c       =zeros(nSc,nTP,nRES)
    end
    if !isnothing(variable_by_name(model_name, "pen_lsh[1,1,1]"))
        nd_load_curt_c   =JuMP.value.(pen_lsh[:,:,:])
    else
        nd_load_curt_c   =zeros(nSc,nTP,nLoads)
    end

    if !isnothing(variable_by_name(model_name, "p_fl_inc[1,1,1]"))
        p_flex_inc_c   =JuMP.value.(p_fl_inc[:,:,:])
    else
        p_flex_inc_c   =zeros(nSc,nTP,nFl)
    end

    if !isnothing(variable_by_name(model_name, "p_fl_dec[1,1,1]"))
        p_flex_dec_c   =JuMP.value.(p_fl_dec[:,:,:])
    else
        p_flex_dec_c   =zeros(nSc,nTP,nFl)
    end
    if !isnothing(variable_by_name(model_name, "p_ch[1,1,1]"))
        p_chrg_c   =JuMP.value.(p_ch[:,:,:])
    else
        p_chrg_c   =zeros(nSc,nTP,nStr_active)
    end
    if !isnothing(variable_by_name(model_name, "p_dis[1,1,1]"))
...
max_mismatch_pf_Q = maximum(abs.(delta_mismatch_inj[(num_PV_buses_pf+num_PQ_buses_pf)+1:end]))
                ...
                (rdata_buses_sw,reactive_gen_sw) = pv_pq_switching(initialization,y_bus_c,nw_qPrf,v_mag_pf,v_angle_pf,rdata_buses,rdata_loads,rdata_gens,idx_oPV_buses_pf,load_bus_type,vol_ctrl_bus_type,mdf_idx_PV_buses_pf,nd_shunt_vec_c,reactive_gen_c)
