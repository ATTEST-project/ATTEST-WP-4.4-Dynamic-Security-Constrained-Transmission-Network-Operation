...
(vol_nodes_mag_pf,vol_nodes_theta_pf,p_gen_node_pf,q_gen_node_pf,convergence_indicator)=ac_power_flow_model(initialization,slack_bus_type,vol_ctrl_bus_type,load_bus_type,
nBus,rdata_buses,
nw_gens_pf,
v_mag_pf,v_angle_pf,
...
push!(pf_result_normal,(vol_nodes_mag_pf,vol_nodes_theta_pf,p_gen_node_pf,q_gen_node_pf))
...
if isempty(idx_conv_normal)
    println("The power flow has converged successfully for all noraml cases")
end
...
elseif indicator=="initial"
    ...
    push!(pf_result_normal,(vol_nodes_mag_pf,vol_nodes_theta_pf,q_gen_node_pf))
    push!(idx_converge_normal, convergence_indicator)
...
max_mismatch_pf_Q = maximum(abs.(delta_mismatch_inj[(num_PV_buses_pf+num_PQ_buses_pf)+1:end]))
             if max_mismatch_pf_P<0.05
                (rdata_buses_sw,reactive_gen_sw) = pv_pq_switching(initialization, y_bus,nw_qPrf,v_mag_pf,v_angle_pf,rdata_buses,rdata_loads,rdata_gens,idx_oPV_buses_pf,load_bus_type,vol_ctrl_bus_type,mdf_idx_PV_buses_pf,nd_shunt_vec,reactive_gen)
...
## Print message whether the power flow algorithm is solved correctly or not ###
            ...
            elseif max_mismatch_pf>epsilon && iteration >= itr_max
                println("The power flow has NOT converged for time $t in the required number of iteration")
                # println("The maximum mismatch is $max_mismatch_pf")
...
#     for t in 1:nTP_pf
        ...
        for i in 1:size(rdata_buses,1)
            ...
            p_inj = vol_nodes_mag_pf[idx_nd_bsheet,1]*sum(v_mag_aux.*((real(y_bus[idx_nd_bsheet,:])).*cos.(theta_diff)+(imag(y_bus[idx_nd_bsheet,:])).*sin.(theta_diff)),dims=1)
            q_inj = vol_nodes_mag_pf[idx_nd_bsheet,1]*sum(v_mag_aux.*((real(y_bus[idx_nd_bsheet,:])).*sin.(theta_diff)-(imag(y_bus[idx_nd_bsheet,:])).*cos.(theta_diff)),dims=1)
...
#
        for i in 1:size(nw_gens_pf,1)
            ...
            # if !isnothing(idx_nd_lsheet[1])
            #     idx_nd_lsheet = idx_nd_lsheet[1,1]
            # end
            # if bus_type != slack_bus_type
                if !isnothing(idx_nd_lsheet[[i]])  &&  isnothing(idx_nd_shunt[[i]])
                    # p_gen_node_pf[i] = p_node_pf_calc_pf[idx_nd_bsheet[1,1]]+nw_qPrf[idx_nd_lsheet[1,1]]
                    # p_gen_node_pf[i] = values(p_node_pf_calc_pf[[idx_nd_bsheet[1,1]]])+nw_qPrf[idx_nd_lsheet[1,1]]
                    push!(p_gen_node_pf, [i]=>values(p_node_pf_calc_pf[[idx_nd_bsheet[1,1]]])+nw_pPrf[idx_nd_lsheet[[i]]])
                    ...
                    # q_gen_node_pf[i] = q_node_pf_calc_pf[idx_nd_bsheet[1,1]]+nw_loads_pf[idx_nd_lsheet[1,1]].load_Q-nd_shunt[idx_nd_shunt]
                elseif !isnothing(idx_nd_lsheet[[i]]) &&  !isnothing(idx_nd_shunt[[i]])
                    # q_gen_node_pf[i] = q_node_pf_calc_pf[idx_nd_bsheet[1,1]]+nw_qPrf[idx_nd_lsheet[1,1]]-nd_shunt_vec[idx_nd_shunt[1,1]]
                    push!(q_gen_node_pf, [i]=>values(q_node_pf_calc_pf[[idx_nd_bsheet[1,1]]])+nw_qPrf[idx_nd_lsheet[[i]]]-nd_shunt_vec[idx_nd_shunt[[i]]])
                elseif isnothing(idx_nd_lsheet[[i]]) &&  !isnothing(idx_nd_shunt[[i]])
                    # q_gen_node_pf[i] = q_node_pf_calc_pf[idx_nd_bsheet[1,1]]-nd_shunt_vec[idx_nd_shunt[1,1]]
                    push!(q_gen_node_pf, [i]=>values(q_node_pf_calc_pf[[idx_nd_bsheet[1,1]]])-nd_shunt_vec[idx_nd_shunt[[i]]])
                else
                    ...
                    # q_gen_node_pf[i] = q_node_pf_calc_pf[idx_nd_bsheet[1,1]]
                end
            end



    return vol_nodes_mag_pf,vol_nodes_theta_pf,p_gen_node_pf,q_gen_node_pf,convergence_indicator
